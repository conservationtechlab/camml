...
""" Elements to work with detection models.
...
def get_objects(interpreter,
    ...
    Args:
      ...
      score_threshold (float): The score threshold for results. All
      returned results have a score greater-than-or-equal-to this
      value.

      image_scale (float, float): Scaling factor to apply to the
        bounding boxes as (x-scale-factor, y-scale-factor), where each
        factor is from 0 to 1.0.

    Returns:
      A list of :obj:`Object` objects, which each contains the
      detected object's id, score, and bounding box as :obj:`BBox`.

    """
    # If a model has signature, we use the signature output tensor
    # names to parse the results. Otherwise, we parse the results
    # based on some assumption of the output tensor order and size.
